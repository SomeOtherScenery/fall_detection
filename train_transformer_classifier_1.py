...
from network import TransformerClassifier
import numpy as np
import torch
...
def validate(criterion, test_loader, model, epoch):
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        valid_acc, valid_loss, confusion_matrix = validate(criterion, test_loader, model, epoch)
        ...
        if epoch == EPOCHS - 1:
            torch.save(best_model.state_dict(), f"./New_models/transformer_classifier_test_split_1_iter_1_{best_epoch}.pth")
            print("best acc: ", best_acc)
            print("best epoch: ", best_epoch)

        with open("./New_models/train_transformer_classifier_test_split_1_iter_1.csv", 'a+', newline='') as f:
            rowIni = list()
            my_writer = csv.writer(f)
            if not is_header:
                my_writer.writerow(['Epoch', "TN", "FP", "FN", "TP", "train_acc", "train_loss", "valid_acc", "valid_loss"])
                is_header = True
